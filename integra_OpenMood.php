...
while ($row = $query_view->fetch_object())
{
    ...
    {
            echo "<br>E-mail não existe. | ".$email."<br />";
            ...
            cadastraAlunoCurso($email,$timestamp_datainicio,$timestamp_datafinal,$timestamp_datainicio,$timestamp_datainicio,$courseid);
            // Efetua a matricula no curso
            efetuaMatriculaAluno($email,$timestamp_datainicio,$courseid);
    ...
    // SE O ALUNO JA ESTIVER MATRICULADO MAS NAO ESTIVER EM NENHUM CURSO, FAZ A VERIFICAÇÃO POR CURSO E PERÍODO VIGENTE
    if ($sql_linha_result != 0) 
    {
        ...
        $result_courseid = $con->query("SELECT id FROM moodle.mdl_enrol
                                         WHERE courseid = '$courseid' AND enrol='manual'");
        $row = $result_courseid->fetch_assoc();
        $mdl_enrol_id = $row['id'];

        // Recupera o contexto do curso.
        $result_contextCurso = $con->query("SELECT id FROM moodle.mdl_context
        ...
        $verifica_conclusao = $con->query("SELECT COUNT(ue.timeend) AS countrecord
                                       ...
                                       INNER JOIN mdl_user u ON u.id=ue.userid
                                            WHERE e.courseid='$courseid' AND u.id='$idUsuarioMoodle' AND ue.timeend > NOW() AND ue.timeend != 0") or die($mysqli->errno .' - '. $mysqli->error);
        $row_verifica_conclusao = $verifica_conclusao->fetch_assoc();
        $result_verifica_conclusao = $row_verifica_conclusao['countrecord'];
        
        // CONDIÇÃO SE ESTIVER MATRICULADO
        if ($result_verifica_inscricao !== 0)
        {
            ...
            // CONDIÇÃO SE ESTIVER NO PERÍODO VIGENTE - SE CONCLUIU OU NÃO
            if ($row_validade > (strtotime(date("Y-m-d H:i:s"))) ) 
            {
